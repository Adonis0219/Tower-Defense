...
public class DraggableUI : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public void OnBeginDrag(PointerEventData eventData)
    {
        ...
        transform.SetParent(canvas);        // 부모 오브젝트를 최상단 canvas로 설정 -> UI 가장 앞에 보임
        transform.SetAsLastSibling();       // 가장 앞에 보이도록 최상단 canvas의 마지막 자식으로 설정

        // 드래그 가능한 오브젝트가 자신만이 아닌 자식들을 가지고 있을 수도 있기 때문에 CanvasGroup으로 통제
        // 알파값을 .6으로 설정, 광선 충돌 처리가 되지 않도록 한다.
        canvasGroup.alpha = .6f;
        canvasGroup.blocksRaycasts = false;
    }
    
    /// <summary>
    /// 현재 오브젝트를 드래그 중일 때 매 프래임 호출
    /// </summary>
    ...
    {
        // 드래그를 시작하면 부모가 canvas로 설정되있는 상태
        // 드래그를 종료할 때 부모가 그대로 canvas이면 아이템 슬롯이 아닌 엉뚱한 곳에
        // 드롭을 했다는 뜻이기 때문에 드래그 직전에 소속되어 있던 아이템 슬롯으로 아이템 이동
        if (transform.parent == canvas)
        {
            // 마지막에 소속되어있었던 previousParent의 자식으로 설정하고, 해당 위치로 이동
            transform.SetParent (previousParent);
            rect.position = previousParent.GetComponentInParent<RectTransform>().position;
        }

        // 알파값을 1로 되돌리고, 광선 충돌처리가 가능하도록 한다
